...
# In-memory gate cache for demo pipeline
...
def _cmd_check_signal(args: list[str]) -> dict[str, Any]:
    ...
    result = check_signal_dict(signal)
...
def _cmd_seed_signal(args: list[str]) -> dict[str, Any]:
    ...
    if not gate or gate.get("decision") != "APPROVE":
        return {"ok": False, "error": "Signal must be APPROVE before seeding."}
...
def _cmd_publish_signal(args: list[str]) -> dict[str, Any]:
    ...
    gate = _gate_cache.get(signal_id)
    if not gate or gate.get("decision") not in ("APPROVE",):
        return {"ok": False, "error": "Signal must be APPROVE before publish."}
...
def _cmd_close_signal(args: list[str]) -> dict[str, Any]:
    ...
    # Mark signal closed in signals.json
    data = load_json(data_path("signals.json"), {"signals": []})
    for s in data.get("signals", []):
        if s.get("signal_id") == signal_id:
            s["status"] = "closed"
